...
def calculatePrediction(symbol):

    # You can either use webreader or yfinance to load the data from yahoo finance

    df = webreader.DataReader(name=symbol, data_source='yahoo', start=start_date, end=end_date)


    #import yfinance as yf #Alternative package if webreader does not work: pip install yfinance
    #df = yf.download(stockname, start=date_start, end=date_today)
    ...
    plus = '+'; minus = ''
    predict_data = {'today_price':price_today,'predected_price':f'{predicted_price} ({plus if percent > 0 else minus}{percent}%)'}
